...
void GearController::GearDown() {
    ...
    wasUp = false;
    SoftDisablePID();
    if (robot->gearPot->Get() <= GEAR_POT_FORCE_DOWN_THRESHOLD) {  // If at rest - further into the robot
...
void GearController::GearIntake() {
    wasDown = true;
    wasRamp = false;
    wasRest = false;
    wasUp = false;
